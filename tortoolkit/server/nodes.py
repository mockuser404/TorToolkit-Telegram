    ...
    for i in res:
        ...
        if len(folders) > 1:
            ...
            TorNode(folders[-1], is_file=True, parent=previous_node,
                    progress=i.progress, size=i.size, priority=i.priority, file_id=l)
            l += 1
        else:
...
def create_list(par, msg):
    ...
    for i in par.children:
        if i.is_folder:
            ...
            if i.priority == 0:
                msg[
                    0] += f"<input type=\"checkbox\" name=\"filenode_{i.file_id}\"> <label for=\"filenode_{i.file_id}\">{i.name} - {human_readable_bytes(i.size)}</label>"
                msg[0] += f"<input type=\"hidden\" value=\"off\" name=\"filenode_{i.file_id}\">"

            else:
                msg[
                    0] += f"<input type=\"checkbox\" checked name=\"filenode_{i.file_id}\"> <label for=\"filenode_{i.file_id}\">{i.name} - {human_readable_bytes(i.size)}</label>"
                msg[0] += f"<input type=\"hidden\" value=\"off\" name=\"filenode_{i.file_id}\">"
