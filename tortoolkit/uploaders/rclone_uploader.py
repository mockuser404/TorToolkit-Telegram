...
class RcloneUploader(BaseTask):
    ...
    async def execute(self):
        ...
        while True:
            ...
            if os.path.isdir(path):
                ...
                if get_val("RSTUFF"):
                    rclone_copy_cmd = [get_val("RSTUFF"), 'copy', f'--config={conf_path}', str(
                        path), f'{dest_drive}:{new_dest_base}', '-f', '- *.!qB', '--buffer-size=1M', '-P']
                else:
                    rclone_copy_cmd = ['rclone', 'copy', f'--config={conf_path}', str(
                        path), f'{dest_drive}:{new_dest_base}', '-f', '- *.!qB', '--buffer-size=1M', '-P']
                ...
                if get_val("ENABLE_SA_SUPPORT_FOR_GDRIVE") and is_gdrive:
                    while True:
                        ...
                        if data == "":
                            ...
                            mat = re.findall(
                                ".*User.*Rate.*(Limit|Quota).*Exceeded.*", data, re.IGNORECASE)
                            if mat is not None:
                                if len(mat) > 0:
                                    is_rate_limit = True
                                    torlog.info(
                ...
                if is_gdrive:
                    ...
                    if ".sharepoint.com/" in get_val("ONEDRIVE_BASE_FOLDER_URL"):
                        folder_link = get_val(
                            "ONEDRIVE_BASE_FOLDER_URL") + parse.quote("/"+os.path.basename(path))
                    else:
                        gid = await self.get_glink(dest_drive, dest_base, os.path.basename(path), conf_path)
                        ids = gid[1].split("#")
                    ...
                    if od_index:
                        index_link = "{}/{}/".format(od_index.strip(
                            "/"), parse.quote(os.path.basename(path)))
                        index_link = requote_uri(index_link)
                        torlog.info("index link "+str(index_link))
                    self._error_reason = "Uploaded Size:- {}\nUPLOADED FOLDER :-<code>{}</code>\nTo OneDrive.".format(
                        ul_size, os.path.basename(path))
                ...
                TtkUpload().deregister_upload(self._user_msg.chat_id,
                                              self._user_msg.id)  # deregister the upload here
                if not is_rate_limit:
    ...
    async def get_glink(self, drive_name, drive_base, ent_name, conf_path, isdir=True):
        ...
        filter_path = os.path.join(os.getcwd(), str(
        ...
        if isdir:
            if get_val("RSTUFF"):
                get_id_cmd = [get_val(
                    "RSTUFF"), "lsjson", f'--config={conf_path}', f"{drive_name}:{drive_base}", "--dirs-only", "-f", f"+ {ent_name}/", "-f", "- *"]
            else:
                get_id_cmd = ["rclone", "lsjson", f'--config={conf_path}',
                              f"{drive_name}:{drive_base}", "--dirs-only", "-f", f"+ {ent_name}/", "-f", "- *"]
            #get_id_cmd = ["rclone", "lsjson", f'--config={conf_path}', f"{drive_name}:{drive_base}", "--dirs-only", f"--filter-from={filter_path}"]
        else:
            if get_val("RSTUFF"):
                get_id_cmd = [get_val(
                    "RSTUFF"), "lsjson", f'--config={conf_path}', f"{drive_name}:{drive_base}", "--files-only", "-f", f"+ {ent_name}", "-f", "- *"]
            else:
                get_id_cmd = ["rclone", "lsjson", f'--config={conf_path}',
                              f"{drive_name}:{drive_base}", "--files-only", "-f", f"+ {ent_name}", "-f", "- *"]
        ...
        try:
            ...
            torlog.exception(
                "Error Occured while getting id ::- {}".format(stdout))
...
class RcloneController:
    ...
    async def execute(self):
        ...
        self._rclone_up = RcloneUploader(
            self._path, self._user_msg, self._dest_drive)

        status_msg = RcloneStatus(
            self, self._rclone_up, self._user_msg.sender_id)
