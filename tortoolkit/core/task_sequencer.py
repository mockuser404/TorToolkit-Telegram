...
class TaskSequence:
        ...
        if self._task_type == self.LEECH:
            ...
            if choices["ext"] and choices["zip"]:
                if is_archive(dl_path):
                    ext_obj = Extractor(
                        dl_path, prev_update_message, self._user_msg)
                    extracted_path = await ext_obj.execute()
                    if extracted_path is not False:
                        dl_path = extracted_path
                else:
                    arch_obj = Archiver(
                        dl_path,  prev_update_message, self._user_msg, not choices["rclone"])
            ...
            if not choices["rclone"]:
                teleup = TelegramUploader(
                    dl_path, self._user_msg, prev_update_message)
            ...
            if get_val("ENABLE_BETA_YOUTUBE_DL"):
                ytdl_obj = PYTDLControllerNew(
                    self._entity_message, self._user_msg)
            else:
                ytdl_obj = PYTDLController(
                    self._entity_message, self._user_msg)
            dl_path = await ytdl_obj.execute()
            if dl_path is False:
                return
    ...
    async def get_leech_choices(self):
        ...
        if get_val("LEECH_ENABLED"):
            buts.append([KeyboardButtonCallback(
                "To Telegram", data=f"leechselect tg {tsp}")])
    ...
    async def get_leech_choice(self, e, timestamp):
        ...
        lis = [False, None]
        cbak = partial(self.get_leech_choice_callback,
