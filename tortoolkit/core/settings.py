...
async def handle_settings(e, edit=False, msg="", submenu=None, session_id=None):
    ...
    if submenu is None:
        ...
        if get_val("ENABLE_SA_SUPPORT_FOR_GDRIVE"):
            ...
            menu.append(
                [KeyboardButtonCallback(
                    f"{prev} LOADED SAS", f"settings change_drive sas_acc {session_id}")]
            )
            ...
            if "Custom file is loaded." in rcval:
                ...
                for j in conf.sections():
                    ...
                    if "team_drive" in list(conf[j]):
                        menu.append(
                            [KeyboardButtonCallback(
                                f"{prev}{j} - TD", f"settings change_drive {j} {session_id}")]
                        )
                    else:
                        menu.append(
                            [KeyboardButtonCallback(
                                f"{prev}{j} - ND", f"settings change_drive {j} {session_id}")]
                        )
                    ...
                    if var_name == "RCLONE_CONFIG":
                        ...
                        try:
                            ...
                            for i in conf.sections():
                                ...
                                SessionVars.update_var(
                                    "DEF_RCLONE_DRIVE", str(i))
                                break
...
async def get_value(e, file=False):
    ...
    cbak = partial(val_input_callback,
                   o_sender=e.sender_id, lis=lis, file=file)
...
async def get_bool_variable(var_name, msg, menu, callback_name, session_id):
    ...
    if val:
        ...
        menu.append(
            [KeyboardButtonCallback(
                yes+msg, f"settings {callback_name} false {session_id}".encode("UTF-8"))]
        )
    else:
        menu.append(
            [KeyboardButtonCallback(
                no+msg, f"settings {callback_name} true {session_id}".encode("UTF-8"))]
        )
