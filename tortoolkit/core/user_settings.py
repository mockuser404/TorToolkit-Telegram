...
async def handle_user_settings(e, edit=False, msg="", submenu=None, sender_id=None):
    ...
    if submenu is None:
            ...
            if not "not loaded" in rcval:
                ...
                for j in conf.sections():
                    ...
                    if "team_drive" in list(conf[j]):
                        menu.append(
                            [KeyboardButtonCallback(
                                f"{prev}{j} - TD", f"usettings change_drive {j} {sender_id}")]
                        )
                    else:
                        menu.append(
                            [KeyboardButtonCallback(
                                f"{prev}{j} - ND", f"usettings change_drive {j} {sender_id}")]
                        )
        ...
        if thumb is not False:
            menu.append(
                [KeyboardButtonCallback(
                    "Change Thumbnail", f"usettings setthumb {sender_id}".encode("UTF-8"))]
            )
            ...
            menu.append(
                [KeyboardButtonCallback(
                    "Set Thumbnail.", f"usettings setthumb {sender_id}".encode("UTF-8"))]
            )
            await get_sub_menu("Go Back ⬅️", "mainmenu", sender_id, menu)
            menu.append(
                [KeyboardButtonCallback(
...
async def get_value(e, file=False, photo=False):
    ...
    cbak = partial(val_input_callback, o_sender=e.sender_id,
                   lis=lis, file=file, photo=photo)
...
async def get_bool_variable(var_name, msg, menu, callback_name, sender_id):
    ...
    if val:
        ...
        menu.append(
            [KeyboardButtonCallback(
                yes+msg, f"usettings {callback_name} false {sender_id}".encode("UTF-8"))]
        )
    else:
        menu.append(
            [KeyboardButtonCallback(
                no+msg, f"usettings {callback_name} true {sender_id}".encode("UTF-8"))]
        )
