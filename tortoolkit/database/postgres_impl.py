...
class TorToolkitDB(PostgresDB):
    ...
    def set_variable(self, var_name, var_value, update_blob=False, blob_val=None):
        ...
        if cur.rowcount > 0:
            ...
            cur.execute(sql, (var_name, var_value,
                              datetime.datetime.now(), vtype))
...
class UserDB(PostgresDB):
    ...
    def set_variable(self, var, value, user_id):
        ...
        if cur.rowcount > 0:
            ...
            cur.execute(
                insql, (json.dumps(self.shared_users.get(user_id)), user_id))

        else:
            insql = "INSERT INTO ttk_users(user_id, json_data) VALUES(%s, %s)"
            cur.execute(insql, (user_id, json.dumps(
                self.shared_users.get(user_id))))
