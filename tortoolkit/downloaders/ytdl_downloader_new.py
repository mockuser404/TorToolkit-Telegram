    ...
    if data is None:
        ...
        for i in unique_formats.keys():
            ...
            if i == "tiny":
                ...
                # add user id
                cdata = f"ytdlsmenu|{i}|{message1.sender_id}|{suid}|{dest}"
            else:
...
async def handle_ytdl_command(e: MessageLike):
    ...
    buts = [[KeyboardButtonCallback(
        "To Telegram", data=f"ytdlselect tg {tsp}")]]
    if await RcloneUploader().get_config() is not None:
        buts.append(
            [KeyboardButtonCallback(
                "To Drive", data=f"ytdlselect drive {tsp}")]
        )
...
async def handle_ytdl_playlist(e: MessageLike) -> None:
    ...
    try:
        ...
        for i in ["144", "240", "360", "480", "720", "1080", "1440", "2160"]:
            keybr.append([KeyboardButtonCallback(
                text=f"{i}p All videos", data=f"ytdlplaylist|{i}|{suid}|{e.sender_id}|{choice}")])

        keybr.append([KeyboardButtonCallback(text=f"Best All videos",
                                             data=f"ytdlplaylist|best|{suid}|{e.sender_id}|{choice}")])

        keybr.append([KeyboardButtonCallback(text="Best all audio only. [340k]",
                                             data=f"ytdlplaylist|320k|{suid}|{e.sender_id}|{choice}")])
        keybr.append([KeyboardButtonCallback(text="Medium all audio only. [128k]",
                                             data=f"ytdlplaylist|128k|{suid}|{e.sender_id}|{choice}")])
        keybr.append([KeyboardButtonCallback(text="Worst all audio only. [64k]",
...
async def get_ytdl_choice(e, timestamp):
    ...
    lis = [False, None]
    cbak = partial(get_leech_choice_callback,
                   o_sender=e.sender_id, lis=lis, ts=timestamp)
    ...
    async def execute(self):
        ...
        if ytdl_task.is_errored:
            if res is False:
                ...
                omess.edit("Something went wrong, try again later." +
                           str(ytdl_task.get_error_reason()), buttons=None)
