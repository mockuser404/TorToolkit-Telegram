...
class QbittorrentDownloader(BaseTask):
    ...
    async def get_client(self, host=None, port=None, uname=None, passw=None, retry=None) -> qba.TorrentsAPIMixIn:
        ...
        # Qbit client
        client = qba.Client(host=host, port=port,
        ...
        try:
            ...
            torlog.info(
                "Client connected successfully to the torrent server. :)")
            try:
            ...
            torlog.info(
                "Oddly enough the qbittorrent server is not running.... Attempting to start at port {}".format(port))
    ...
    async def add_torrent_magnet(self):
        ...
        try:
            ...
            if len(ext_res) > 0:
                torlog.info(
                    f"This torrent is in list {ext_res} {magnet} {ext_hash}")
            ...
            if op.lower() == "ok.":
                ...
                while True:
                    if (datetime.now() - st).seconds >= 10:
                        torlog.warning(
                            "The provided torrent was not added and it was timed out. magnet was:- {}".format(magnet))
    ...
    async def add_torrent_file(self):
        ...
        try:
            ...
            if len(ext_res) > 0:
                torlog.info(
                    f"This torrent is in list {ext_res} {path} {ext_hash}")
                self._is_errored = True
                self._error_reason = "This torrent is already in the leech list."
                return False
            ...
            if op.lower() == "ok.":
                ...
                while True:
                    if (datetime.now() - st).seconds >= 20:
                        torlog.warning(
                            "The provided torrent was not added and it was timed out. file path was:- {}".format(path))
                        torlog.error(ext_hash)
                        self._is_errored = True
                        self._error_reason = "The torrent was not added due to an error."
                        return False
    ...
    async def resume_all(self):
        ...
        msg += "▶️Resumed {} torrents check the status for more...▶️".format(
            len(tors))
        ...
        while True:
            ...
            try:
                ...
                if (is_meta and (time.time() - meta_time) > get_val("TOR_MAX_TOUT")):
                    ...
                    self._is_errored = True
                    self._error_reason = "Torrent <code>{}</code> is DEAD. [Metadata Failed]".format(
                        tor_info.name)
                    torlog.error(
                        "An torrent has no seeds clearing that torrent now. Torrent:- {} - {}".format(tor_info.hash, tor_info.name))
                    await self.delete_this(tor_info.hash)
                ...
                try:
                    if tor_info.state == "error":
                        ...
                        self._is_errored = True
                        self._error_reason = "Torrent <code>{}</code> errored out.".format(
                            tor_info.name)
                        torlog.error(
                            "An torrent has error clearing that torrent now. Torrent:- {} - {}".format(tor_info.hash, tor_info.name))
                        await self.delete_this(tor_info.hash)
                    ...
                    if tor_info.state == "uploading" or tor_info.state.lower().endswith("up"):
                        ...
                        self._error_reason = "Download completed: `{}` - (`{}`)\nTo path: `{}`".format(
                            tor_info.name, human_readable_bytes(tor_info.total_size), tor_info.save_path)
                        self._is_done = True
                        return savepath
                    else:
    ...
    async def execute(self):
        ...
        if self._is_file:
            ...
            if self._qbit_task.is_errored:
                self._update_message("Your Task was unsccuessful. {}".format(
        ...
        self._update_message = await self._update_message.edit("Download will be automatically started after 180s of no action.", buttons=[
            ...
            [
                KeyboardButtonCallback(
                    "Done Selecting Files.", data=f"doneselection {self._user_msg.sender_id} {self._user_msg.id}".encode("UTF-8"))
            ]
        ])
        ...
        status_mgr = QbittorrentStatus(
            self, self._qbit_task, self._user_msg.sender_id)
    ...
    def get_magnets(self, text):
        ...
        for i in range(len(matches)):
            if i == len(matches)-1:
                ...
                magnets.append(
                    text[matches[i].span()[0]:matches[i+1].span()[0]])
